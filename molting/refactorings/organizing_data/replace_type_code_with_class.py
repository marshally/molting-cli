...
class ReplaceTypeCodeWithClass(RefactoringBase):
    ...
    def _extract_type_codes(self, class_node: ast.ClassDef) -> dict[str, Any]:
        """Extract type code constants from the class.
            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        elif isinstance(node.value, ast.Num):  # For older Python AST
                            type_codes[target.id] = node.value.n

        return type_codes

    def _create_type_code_class(self, type_codes: dict[str, Any]) -> ast.ClassDef:
    ...
    def _remove_type_codes(self, class_node: ast.ClassDef) -> None:
        ...
        Args:
            class_node: The AST ClassDef node of the target class
        """
        # Remove assignments that are type codes (all caps variable names with constant values)
        nodes_to_remove = []
        for node in class_node.body:
            if isinstance(node, ast.Assign):
                for target in node.targets:
    ...
    def _create_type_code_assignments(self, type_codes: dict[str, Any]) -> list[ast.Assign]:

...
class TestIntroduceAssertion(RefactoringTestBase):
    ...
    def test_invalid_target_format(self):
        ...
        from molting.refactorings.simplifying_conditionals.introduce_assertion import (
            IntroduceAssertion,
        )
        ...
        try:
            with pytest.raises(ValueError, match="Invalid target format"):
                IntroduceAssertion(temp_file, target="divide_without_line", condition="b != 0")
        finally:
...
class TestIntroduceAssertionCLI:
    ...
    def test_cli_command_exists(self):
        ...
        assert (
            "introduce-assertion" in result.output or "Make assumptions explicit" in result.output
        )

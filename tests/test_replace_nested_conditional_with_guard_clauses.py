...
class TestReplaceNestedConditionalWithGuardClauses(RefactoringTestBase):
    ...
    def test_simple_nested_conditional(self):
        ...
        self.refactor("replace-nested-conditional-with-guard-clauses", target="get_payment#L2")

    def test_class_method(self):
        """Convert nested conditional in a class method."""
        self.refactor(
            "replace-nested-conditional-with-guard-clauses", target="Order::get_discount#L3"
        )

    def test_multi_level_nested(self):
        """Convert deeply nested if-else (4 levels) to guard clauses."""
        self.refactor("replace-nested-conditional-with-guard-clauses", target="process_status#L2")
    ...
    def test_invalid_target_format(self):
        ...
        try:
            with pytest.raises(ValueError, match="Invalid target format"):
                ReplaceNestedConditionalWithGuardClauses(temp_file, target="function_without_line")
        finally:

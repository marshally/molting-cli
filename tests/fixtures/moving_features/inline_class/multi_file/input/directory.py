...
def print_directory_entry(person: Person):
    ...
    print(f"Phone: ({person.phone_number.area_code}) {person.phone_number.number}")


def search_by_area_code(people: list, area_code: str):
    """Find all people with a given area code."""
    return [p for p in people if p.phone_number.area_code == area_code]
...
def format_contact_list(people: list):
    ...
    for person in people:
        entry = f"{person.name}: ({person.phone_number.area_code}) {person.phone_number.number}"
...
def get_phone_parts(person: Person):
    ...
    return person.phone_number.get_area_code(), person.phone_number.get_number()
